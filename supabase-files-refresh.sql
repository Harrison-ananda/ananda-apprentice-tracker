-- ananda apprentice tracker file refresh
-- Run this in Supabase SQL Editor so uploaded apprentice files survive refresh.
...
begin
  ...
  return query
  insert into public.apprentice_files (id, apprentice_id, file_name, file_type, file_size, data_url, updated_at)
  values (
    coalesce(input_file_id, gen_random_uuid()),
    input_apprentice_id,
    coalesce(input_file_name, 'file'),
    coalesce(input_file_type, 'file'),
    coalesce(input_file_size, 0),
    coalesce(input_data_url, ''),
    now()
  )
...
grant execute on function public.save_file_staff(uuid, uuid, text, text, integer, text) to authenticated;
